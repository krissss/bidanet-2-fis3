...
api.getSelfData(function(data){
  $.each(data.msgs, function(index, item){
    if(item.msgTypeCode != 5 || item.customer_user_status != 5){ // msgTypeCode 5 为门市新客待接收
      logger('非新客消息跳过', item);
      return true;
    }
...
// 接收新客资
function acceptCustomer(uuid){
  api.saveObject('BusCustomer', {
    uuid: uuid,
    seq: seq, // 传递seq 用于删除该条消息
    customer_user_status: 6,
    mscreate_time: new Date().getTime(),
...
$('body').on('click', '.accept-customer', function(){
  ...
  api.getOperationUserAndStatus(id, function(data){
    if(data.sales_id == api.userInfo.userUuid && data.customer_user_status == 5){ // 门市是自己，并且状态是5才可以接收
